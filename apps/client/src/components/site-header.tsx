...
export function SiteHeader({ overlay = false }: { overlay?: boolean }) {
  ...
  const transparent = overlay && !scrolled; // white text floating over the hero
  const navLink = transparent ? "text-white/85 hover:text-white" : "text-ink-soft hover:text-ink";

  return (
    <header className={cn(
      "z-30 transition-colors duration-300",
      overlay
        ? cn("fixed inset-x-0 top-0", scrolled ? "border-b border-ink/8 bg-sand/85 backdrop-blur-xl" : "bg-transparent")
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3.5">
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle className={transparent ? "h-9 w-9 text-white/85 hover:bg-white/15 hover:text-white" : "h-9 w-9"} />
          {isLoading ? (
            ...
            <div className="relative">
              <button onClick={() => setOpen((o) => !o)} onBlur={() => setTimeout(() => setOpen(false), 150)}
                className={cn("flex items-center gap-2 rounded-full py-1 pl-1 pr-2.5 text-sm font-medium transition-colors",
                  transparent ? "border border-white/25 bg-white/10 text-white hover:bg-white/20" : "border border-ink/12 bg-paper hover:border-ink/25")}>
                <span className="grid h-7 w-7 place-items-center rounded-full bg-strong text-xs font-bold text-white">
                  {(user.email[0] ?? "U").toUpperCase()}
                </span>
                <ChevronDown size={14} className={transparent ? "text-white/70" : "text-ink-soft"} />
              </button>
              {open && (
            ...
            <>
              ...
              <Link href="/sign-up"><Button size="sm" className="rounded-md text-white">Créer un compte</Button></Link>
            </>
          )}
        </div>
      </div>
