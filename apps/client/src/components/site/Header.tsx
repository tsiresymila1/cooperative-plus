            ...
            {NAV.map((item) => (
              <li key={item.label} className="relative">
                <Link
                  ...
                  className={`relative flex h-[26px] items-center px-5 text-[14px] font-medium transition-colors duration-[400ms] ease-out hover:text-gold-nav ${pathname === item.href ? "text-gold-nav" : linkColor}`}
                >
                  {item.label}
                  {/* {pathname === item.href ? (
                    <span
                      aria-hidden
                      className="absolute inset-x-5 bottom-0 h-px animate-[navUnderline_500ms_ease_forwards] bg-gold-nav"
                    />
                  ) : null} */}
                </Link>
              </li>
        ...
        <div className="flex items-center gap-x-[18px]">
          {authed ? (
              ...
              {menu ? (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white py-2 shadow-[0_10px_30px_rgba(20,49,76,0.14)]">
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2.5 px-4 py-2.5 text-[14px] text-navy hover:bg-gold hover:text-navy"
                  >
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2.5 px-4 py-2.5 text-[14px] text-navy hover:bg-gold hover:text-navy"
                  >
                    <Ticket className="size-4" /> Mes réservations
                  </Link>
                  <button
                    onClick={async () => {
                      if (
            ...
            <Link
              ...
              className={`hidden items-center gap-1.5 text-[14px] font-medium transition-colors duration-500 hover:text-gold-hover sm:flex ${linkColor}`}
            >
          ...
          <Link
            href="/search"
            className="hidden h-[44px] items-center bg-gold px-5 font-display text-[15px] font-semibold uppercase tracking-[0.5px] text-navy transition-colors duration-[250ms] hover:bg-navy hover:text-white sm:inline-flex"
          >
            Réserver
          </Link>
          ...
          <button
            ...
            aria-label="Menu"
            className={`transition-colors duration-300 hover:text-gold lg:hidden ${linkColor}`}
          >
        ...
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-[300px] max-w-[85vw] overflow-y-auto bg-white p-6">
            <button
              ...
              onClick={() => setOpen(false)}
              aria-label="Fermer"
              className="mb-6 text-navy transition-colors hover:text-gold"
            >
              ...
              {NAV.map((item) => (
                <li key={item.label}>
                  <Link
                    href={item.href}
                    onClick={() => setOpen(false)}
                    className="flex items-center justify-between py-2.5 text-[15px] font-medium text-navy transition-colors hover:text-gold"
                  >
                    {item.label}{" "}
                    <ChevronRight className="size-4 text-navy/40" />
                  </Link>
                </li>
              ))}
              <li className="pt-3">
                {authed ? (
                  <Link
                    href="/account/dashboard"
                    onClick={() => setOpen(false)}
                    className="block bg-gold px-4 py-3 text-center font-display font-semibold uppercase text-navy"
                  >
                    Mon compte
                  </Link>
                ) : (
